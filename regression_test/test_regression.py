...
import datetime
from dataset4regress_test import get_data
import dataset4regress_test
...
def test(device, optimizer, learner, test_data, loss_func):
    ...
    if False:
        ...
        for i in range(len(target_total_test)):
            ...
            if not os.path.exists(dataset4regress_test.dataset_directory + '/' + dataset4regress_test.dataset_folder[1:] + '/log/test_{0:%m%d}_{0:%H%M}/false_pred_{1}/{2}/{3}/'.format(now, 0, np.array(target_total_test)[i], np.array(int_y_total)[i])):
                 os.makedirs(dataset4regress_test.dataset_directory + '/' + dataset4regress_test.dataset_folder[1:] + '/log/test_{0:%m%d}_{0:%H%M}/false_pred_{1}/{2}/{3}/'.format(now, 0, np.array(target_total_test)[i], np.array(int_y_total)[i]))
            shutil.copyfile(paths_total[i], dataset4regress_test.dataset_directory + '/' + dataset4regress_test.dataset_folder[1:] + '/log/test_{0:%m%d}_{0:%H%M}/false_pred_{1}/{2}/{3}/'.format(now, 0, np.array(target_total_test)[i], np.array(int_y_total)[i]) + splited_path[-1])
            bar.update()
        bar.close()

    draw_graph.yyplot_density(np.array(target_total_test), np.array(y_total_test), False, save_place=dataset4regress_test.dataset_directory + '/' + dataset4regress_test.dataset_folder + '/log/test_{0:%m%d}_{0:%H%M}/'.format(now, now))
    draw_graph.plot_confusion_matrix(target_total_test, int_y_total, dataset4regress_test.num_list, save_caption=dataset4regress_test.dataset_folder, save_place=dataset4regress_test.dataset_directory + dataset4regress_test.dataset_folder + '/log/test_{0:%m%d}_{0:%H%M}/'.format(now, now))  
...
def main(learner):
    ...
    if not os.path.exists(dataset4regress_test.dataset_directory + dataset4regress_test.dataset_folder + '/log/test_{0:%m%d}_{0:%H%M}'.format(now, now)):
        os.makedirs(dataset4regress_test.dataset_directory + dataset4regress_test.dataset_folder + '/log/test_{0:%m%d}_{0:%H%M}'.format(now, now))
