...
files = "/dataset/resized/loc-6/loc-6-extensive-extracted-raster_output_x_x_x_x_x_resized_48_48"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/6/*")
test_files = glob.glob(full_path + "/test/6/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*6])

        if self.transform is not None:
            img = self.transform(img)

        location = [self.y[index*6], self.y[index*6+1], self.y[index*6+2], self.y[index*6+3], self.y[index*6+4], self.y[index*6+5]]
        path = self.img_paths[index*6]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/6)
...
def get_data(batch_size):
    ...
    normalize = transforms.Normalize(mean=[9.538565, 7.0146775, 1.6524022], std=[0.24117401, 0.23618957, 0.2502744]) # for 48
    #normalize = transforms.Normalize(mean=[-37.91903, -28.006096, -4.984318], std=[0.8857655, 0.9037094, 0.8260992]) # for 64
