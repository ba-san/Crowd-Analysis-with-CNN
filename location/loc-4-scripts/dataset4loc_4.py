...
files = "/dataset/resized/loc-4/loc-4-extensive-extracted-raster_output_x_x_x_x_x_resized_x_x"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/4/*")
test_files = glob.glob(full_path + "/test/4/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*4])

        if self.transform is not None:
            img = self.transform(img)

        location = [self.y[index*4], self.y[index*4+1], self.y[index*4+2], self.y[index*4+3]]
        path = self.img_paths[index*4]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/4)
...
def get_data(batch_size):
    ...
    normalize = transforms.Normalize(mean=[2.3816075, 1.8510833, 0.6851765], std=[0.24862227, 0.24394591, 0.26072365])
