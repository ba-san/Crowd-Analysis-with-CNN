...
files = "/dataset/resized/loc-3/loc-3-extensive-extracted-raster_output_x_x_x_x_x_resized_x_x"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/3/*")
test_files = glob.glob(full_path + "/test/3/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*3])

        if self.transform is not None:
            img = self.transform(img)

        #location = self.y[index]
        location = [self.y[index*3], self.y[index*3+1], self.y[index*3+2]]
        path = self.img_paths[index*3]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/3)
