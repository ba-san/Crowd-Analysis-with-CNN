...
files = "/dataset/resized/loc-8/loc-8-extensive-extracted-raster_output_x_x_x_x_x_resized_x_x"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/8/*")
test_files = glob.glob(full_path + "/test/8/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*8])

        if self.transform is not None:
            img = self.transform(img)

        location = [self.y[index*8], self.y[index*8+1], self.y[index*8+2], self.y[index*8+3], self.y[index*8+4], self.y[index*8+5], self.y[index*8+6], self.y[index*8+7]]
        path = self.img_paths[index*8]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/8)
...
def get_data(batch_size):
    ...
    normalize = transforms.Normalize(mean=[9.489424, 6.9654465, 1.6031768], std=[0.23163208, 0.2270195, 0.24118082])
