...
files = "/dataset/resized/loc-9/loc-9-extensive-extracted-raster_output_x_x_x_x_x_resized_48_48"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/9/*")
test_files = glob.glob(full_path + "/test/9/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*9])

        if self.transform is not None:
            img = self.transform(img)

        location = [self.y[index*9], self.y[index*9+1], self.y[index*9+2], self.y[index*9+3], self.y[index*9+4], self.y[index*9+5], self.y[index*9+6], self.y[index*9+7], self.y[index*9+8]]
        path = self.img_paths[index*9]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/9)
...
def get_data(batch_size):
    ...
    normalize = transforms.Normalize(mean=[9.467754, 6.943619, 1.581425], std=[0.22345617, 0.21991807, 0.23474833])
    ...
    train_data = torch.utils.data.DataLoader(train_dataset, batch_size = batch_size, shuffle = True, num_workers = 2, pin_memory = True)
    test_data = torch.utils.data.DataLoader(test_dataset, batch_size = batch_size, shuffle = False, num_workers = 2, pin_memory = True)
