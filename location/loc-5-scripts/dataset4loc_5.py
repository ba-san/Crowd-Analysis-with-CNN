...
files = "/dataset/resized/loc-5/loc-5-extensive-test-extracted-raster_output_x_x_x_x_x_resized_48_48"

full_path = PWD + files
train_files = glob.glob(full_path + "/train/5/*")
test_files = glob.glob(full_path + "/test/5/*")
...
class LocationDataset(Dataset):
    ...
    def __getitem__(self, index):
        img = Image.open(self.img_paths[index*5])

        if self.transform is not None:
            img = self.transform(img)

        location = [self.y[index*5], self.y[index*5+1], self.y[index*5+2], self.y[index*5+3], self.y[index*5+4]]
        path = self.img_paths[index*5]

        return img, location, path

    def __len__(self):
        return int(self.img_paths.shape[0]/5)
...
def get_data(batch_size):
    ...
    #normalize = transforms.Normalize(mean=[-7.8660765, -5.8048053, -0.9384776], std=[0.86555976, 0.8840436, 0.8109293])
    normalize = transforms.Normalize(mean=[9.562461, 7.04071, 1.6782736], std=[0.24846533, 0.24359617, 0.25869617])  # loc-5-extensive-test-extracted-raster_output_x_x_x_x_x_resized_48_48
