...
import datetime
from dataset4loc_7 import get_data
import dataset4loc_7
...
def train(device, optimizer, learner, train_data, loss_func):
    ...
    if epoch==999:
        ...
        if os.path.exists(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/train-pred-{1}/'.format(now, epoch)):
            shutil.rmtree(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/train-pred-{1}/'.format(now, epoch))
        os.makedirs(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/train-pred-{1}/'.format(now, epoch))

        bar = tqdm(desc = "dotting", total = extract_num, leave = False)
        f = open(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/train-pred-{1}/'.format(now, epoch) + str(epoch) + '-pred-locations.txt', 'w')
        ...
        for i in random_extract_list:
            ...
            img_lists = glob.glob(dataset4loc_7.full_path + '/train/*/*'.format(now, epoch))
            ...
            and 0<=int(loc_total_train[i][12])<img.shape[0] and 0<=int(loc_total_train[i][13])<img.shape[1]:
                ...
                for k in range(num_loc):
                    ...
                    if int(loc_total_train[i][2*k])-1 >= 0:    
                        img[int(loc_total_train[i][2*k+1]), int(loc_total_train[i][2*k])-1] = colors[k]
            cv2.imwrite(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/train-pred-{1}/'.format(now, epoch) + new_img_file[-1], img)
            bar.update()
        bar.close()
        f.close()
...
def test(device, optimizer, learner, test_data, loss_func):
    ...
    if (epoch>1 and epoch%40==0) or epoch==199:
        ...
        hp_for_record= get_arguments()
        bs = hp_for_record["batch_size"]
        if os.path.exists(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/test-pred-{1}/'.format(now, epoch)):
            shutil.rmtree(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/test-pred-{1}/'.format(now, epoch))
        os.makedirs(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/test-pred-{1}/'.format(now, epoch))

        bar = tqdm(desc = "dotting", total = extract_num, leave = False)
        f = open(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/test-pred-{1}/'.format(now, epoch) + str(epoch) + '-pred-locations.txt', 'w')

        random_extract_list = random.sample(range(len(paths_total)), k=extract_num)

        for i in random_extract_list:
            path_separated = paths_total[i].split('/')
            img_lists = glob.glob(dataset4loc_7.full_path + '/test/*/*'.format(now, epoch))
            img_file = [s for s in img_lists if path_separated[-1] in s]
            img = cv2.imread(img_file[0])
            new_img_file = img_file[0].split('/')
            ...
            and 0<=int(loc_total_test[i][12])<img.shape[0] and 0<=int(loc_total_test[i][13])<img.shape[1]:
                ...
                for k in range(num_loc):
                    ...
                    if int(loc_total_test[i][2*k])-1 >= 0:    
                        img[int(loc_total_test[i][2*k+1]), int(loc_total_test[i][2*k])-1] = colors[k]
            cv2.imwrite(dataset4loc_7.full_path + '/log/{0:%m%d}_{0:%H%M}/test-pred-{1}/'.format(now, epoch) + new_img_file[-1], img)
            bar.update()
        bar.close()
        f.close()
...
def main(learner):
    ...
    if not os.path.exists(dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/models'.format(now)):
        os.makedirs(dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/models'.format(now))
        shutil.copyfile("./train_loc_7.py", dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/train_loc_7_{0:%m%d}_{0:%H%M}.py'.format(now))
        shutil.copyfile("./dataset4loc_7.py", dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/dataset4loc_7_{0:%m%d}_{0:%H%M}.py'.format(now))
    ...
    for epoch in range(learner.module.epochs):
        ...
        with torch.no_grad():
            ...
            if best_loss is None:
                ...
                best_loss = test_loss
                counter = 0
                torch.save(learner.module.state_dict(), dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/models/'.format(now, now) + os.path.basename(dataset4loc_7.dataset_folder) + '_{0:%m%d}_{0:%H%M}_{1}.pth'.format(now, epoch))
            ######################
        ...
        draw_graph.draw_graph_regress(learner.module.epochs, epoch, train_loss, test_loss, os.path.basename(dataset4loc_7.dataset_folder), save_place=dataset4loc_7.dataset_directory  + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/'.format(now, now), ymax=300.0)
        ...
        save_place = dataset4loc_7.dataset_directory + dataset4loc_7.dataset_folder + '/log/{0:%m%d}_{0:%H%M}/'.format(now, now)
        write_gspread.update_gspread(dataset4loc_7.dataset_folder, 'WRN', dataset4loc_7.dataset_directory, now, 'N/A(regress)', train_loss, 'N/A(regress)', test_loss, epoch+1, learner.module.epochs, False, save_place, otherparams)
        
        print_result(rsl[-1].values())
        scheduler.step()
          
        if earlystopper == 1:
            write_gspread.update_gspread(dataset4loc_7.dataset_folder, 'WRN', dataset4loc_7.dataset_directory, now, 'N/A(regress)', train_loss, 'N/A(regress)', test_loss, epoch+1, learner.module.epochs, True, save_place, otherparams)
            break
